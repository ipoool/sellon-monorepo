...
setup_caddy() {
  ...
  # TLS for the PLATFORM domain. Two modes:
  #   - Behind Cloudflare proxy (orange cloud): Caddy CANNOT complete the public
  #     ACME challenge (CF terminates TLS at its edge), so it must serve a
  #     Cloudflare Origin Certificate. Drop the cert + key here and Caddy uses
  #     them; set CF SSL/TLS mode to "Full (strict)".
  #         CF dashboard → SSL/TLS → Origin Server → Create Certificate
  #         hostnames: ${DOMAIN}, *.${DOMAIN}
  #   - DNS-only (grey cloud): leave these files absent and Caddy gets a real
  #     Let's Encrypt cert automatically.
  # Seller CUSTOM domains are unaffected either way — they are DNS-only (CNAME
  # to cname.sellon.id) so they hit the origin directly and use on-demand LE.
  ORIGIN_CERT="${ORIGIN_CERT:-/etc/caddy/origin/tls.pem}"
  ORIGIN_KEY="${ORIGIN_KEY:-/etc/caddy/origin/tls.key}"
  local platform_tls=""
  if [[ -f "$ORIGIN_CERT" && -f "$ORIGIN_KEY" ]]; then
    ...
    warn "cert Let's Encrypt-nya. Buat Cloudflare Origin Certificate dulu:"
    warn "  CF dashboard → SSL/TLS → Origin Server → Create Certificate"
    warn "  hostnames: ${DOMAIN}, *.${DOMAIN}"
    warn "  simpan cert → ${ORIGIN_CERT}, key → ${ORIGIN_KEY} (chmod 600), set CF mode Full (strict),"
    warn "  lalu jalankan ulang: sudo DOMAIN=${DOMAIN} LETSENCRYPT_EMAIL=${LETSENCRYPT_EMAIL} bash $(basename "$0") setup_caddy"
    warn "(Tanpa origin cert: platform domain pakai ACME otomatis — HANYA jalan kalau DNS-only.)"
  fi

  # Write the Caddyfile. ${DOMAIN}/${LETSENCRYPT_EMAIL}/${platform_tls} are
  # substituted now; the rest is literal Caddy config (no shell vars inside).
...
# Platform domain. With an origin cert present, Caddy serves it (Cloudflare
# proxy + Full strict); otherwise Caddy obtains a Let's Encrypt cert at startup
# (only works if the domain is DNS-only).
